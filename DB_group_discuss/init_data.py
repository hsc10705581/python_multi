...
def init_job(mydb, mycursor, size, size_of_user):
    ...
    for i in range(size):
        ...
        val = (salary, rand_str, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(begin_time)),
               time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_time)), index, random.randint(0, size_of_user))
        try:
...
def init_apply(mydb, mycursor, size, size_of_user, size_of_job):
    a1 = (1976, 1, 1, 0, 0, 0, 0, 0, 0)  # 设置开始日期时间元组（1976-01-01 00：00：00）
    a2 = (2020, 12, 31, 23, 59, 59, 0, 0, 0)  # 设置结束日期时间元组（2020-12-31 23：59：59）
    start = time.mktime(a1)  # 生成开始时间戳
    end = time.mktime(a2)  # 生成结束时间戳
...
def init_follow_black(mydb, mycursor, size, size_of_user):
    sql = "insert into follow_unfollow_black (emp_user_id, user_id, follow, black) VALUES (%s, %s, %s, %s)"
    for i in range(size):
        rand_employer = random.randint(0, size_of_user)
        rand_employee = random.randint(0, size_of_user)
...
def init_send_receive(mydb, mycursor, size, size_of_user):
    a1 = (1976, 1, 1, 0, 0, 0, 0, 0, 0)  # 设置开始日期时间元组（1976-01-01 00：00：00）
    a2 = (2020, 12, 31, 23, 59, 59, 0, 0, 0)  # 设置结束日期时间元组（2020-12-31 23：59：59）
    start = time.mktime(a1)  # 生成开始时间戳
    end = time.mktime(a2)  # 生成结束时间戳
...
def init_skill_tag(mydb, mycursor, size, size_of_user, size_of_job):
    default_skill = ["java", "python", "c", "c++", "mysql", "c#", "go", "javascript"]
...
def main():
    mydb = pymysql.connect("localhost", "root", "root", "zpapp")
    mycursor = mydb.cursor()

    size_of_admin = 10 #admin表的大小
    print("init admin of size: " + str(size_of_admin))
    init_admin(mydb, mycursor, size_of_admin)

    size_of_user = 1000 #user表的大小，同时生成继承自它的employer和employee
    print("init user, employee and employer of size: " + str(size_of_user))
    init_user(mydb, mycursor, size_of_user)

    temp_size_of_job = 10000 #job表的尝试次数
    print("init job of size: " + str(temp_size_of_job))
    size_of_job = init_job(mydb, mycursor, temp_size_of_job, size_of_user)

    temp_size_of_comment = 1000 #comment表的尝试次数
    ...
    size_of_comment_love_report = 1000  # job_love_report表的大小
    ...
    size_of_job_love_report = 1000 #job_love_report表的大小
    ...
    size_of_user_love_report = 1000 #user_love_report表的大小
    ...
    init_apply(mydb, mycursor, size_of_apply, size_of_user, size_of_job)

    size_of_follow = 10000
    print("init follow_unfollow_black of size: " + str(size_of_follow))
    init_follow_black(mydb, mycursor, size_of_follow, size_of_user)

    size_of_message = 10000
    print("init send_receive_message of size: " + str(size_of_message))
    init_send_receive(mydb, mycursor, size_of_message, size_of_user)

    size_of_skill = 10000
    print("init skill_tag, employee_skill and job_skill of size: " + str(size_of_skill))
    init_skill_tag(mydb, mycursor, size_of_skill, size_of_user, size_of_job)
